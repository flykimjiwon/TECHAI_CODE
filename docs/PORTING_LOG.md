...
> 대상: `github.com/kimjiwon/tgc` (택갈이코드)
...
- **포팅 파일들**:
  - `택갈이코드/internal/llm/prompt.go` — 전면 재작성
  - `택갈이코드/internal/llm/prompts/core.md` — hanimo에서 복사
  - `택갈이코드/internal/llm/prompts/super.md` — hanimo 기반, 도구 목록 확장 (hashline, git, diagnostics, knowledge_search)
  - `택갈이코드/internal/llm/prompts/dev.md` — hanimo 기반, 도구 목록 확장
  - `택갈이코드/internal/llm/prompts/plan.md` — hanimo 기반, 읽기전용→쓰기가능으로 변경
  - `택갈이코드/internal/llm/prompts/askuser.md` — hanimo에서 복사
- **주요 변경**:
...
- **포팅**: `택갈이코드/internal/llm/environment.go` (148줄)
...
- **포팅**: `택갈이코드/internal/knowledge/userdocs.go` (221줄)
- **변경**:
...
- **포팅**: `택갈이코드/internal/tools/registry.go`에 추가
...
- **포팅**: `택갈이코드/internal/app/app.go` 수정
...
- **포팅**: `택갈이코드/internal/ui/styles.go`에 추가
...
| 기능 | hanimo 파일 | 이유 |
|------|------------|------|
| MCP 클라이언트 | `internal/mcp/` | 택갈이코드는 내부망 전용, MCP 불필요 |
| 멀티 프로바이더 | `internal/llm/providers/` | 택갈이코드는 OpenAI-compat 단일 엔드포인트 |
| 테마 시스템 | `internal/ui/styles.go` Themes | 우선순위 낮음 |
| 페르소나 | `internal/ui/persona.go` | 우선순위 낮음 |
| i18n 다국어 | `internal/ui/i18n.go` | 택갈이코드는 한국어 고정 |
