...
- **헤드리스 Exec**: `techai exec "prompt"` — TUI 없이 CLI 실행, 파이프 stdin 지원, CI/CD 자동화 통합
- **Lifecycle Hooks**: 도구 실행 전후에 자동 lint/감사/알림 — hooks.json 설정만으로 확장
- **apply_patch**: 멀티파일 패치 — 한 번의 호출로 여러 파일 생성/수정/삭제/이동 (Codex 포맷)
...
- **Knowledge Packs**: 74개 내장 지식 문서 + 프로젝트별 ON/OFF 선택 + 사용자 문서 추가 가능
...
multi:
  enabled: false       # 현재 비활성화 (추후 재오픈 예정)
...
## Knowledge Packs (지식 문서 시스템)

74개의 기술 문서가 바이너리에 내장되어 있습니다. 프로젝트에 필요한 팩만 선택하면 해당 문서만 AI 컨텍스트에 주입됩니다.
...
프로젝트 타입을 감지하여 `.techai.md`에 관련 팩을 자동 추천합니다:
...
```markdown
# 필요한 팩만 선택 (쉼표로 구분)
knowledge_packs: react, database, java

# 전부 끄기 (지식 없이 사용)
knowledge_packs: none

# 줄 자체를 삭제하면 → 74개 전부 로드
```

**3. 사용자 지식 문서 추가**

`.tgc/knowledge/` 폴더에 `.md` 파일을 넣으면 자동 인덱싱됩니다:

```
.tgc/knowledge/
├── my-api-guide.md       ← 사용자가 직접 작성
...
| 팩 | 파일 수 | 내용 |
|----|--------|------|
| `react` | 11 | React 19, Next.js, Hook Form, TanStack Query, Zustand |
| `database` | 9 | SQL Core, PostgreSQL, MySQL, Oracle, Tibero, Prisma, Drizzle |
| `java` | 7 | Spring (Core/MVC/Security/JPA/Boot/Test), JEUS |
| `bxm` | 13 | BXM 프레임워크 (배치, Bean, DBIO, Service 등) |
| `terminal` | 11 | Bash, Git, Linux/macOS/Windows, 크로스플랫폼 |
| `css` | 5 | Tailwind v4, Framer Motion, Radix UI, shadcn/ui |
| `charts` | 4 | Chart.js, D3, ECharts, Recharts |
| `testing` | 3 | Vitest, Testing Library, Playwright |
| `tooling` | 3 | ESLint, Turborepo, Vite |
| `go` | 1 | Go 표준 라이브러리 |
| `javascript` | 1 | ES2024+ |
| `typescript` | 1 | TypeScript 타입 시스템 |
| `python` | 1 | FastAPI |
| `vue` | 1 | Vue 3 Composition API |
| `auth` | 1 | Auth.js (NextAuth) |
| `validation` | 1 | Zod |
| `utils` | 1 | date-fns |
...
```
사용자 질문: "React Hook Form으로 폼 만들어줘"
      ↓
키워드 추출: "react", "hook", "form"
      ↓
인덱싱된 팩에서 매칭 문서 검색 (~1ms)
      ↓
관련 문서 내용을 시스템 프롬프트에 주입 (최대 8192 토큰)
      ↓
모델이 내장 지식을 참고하여 답변
```

> 벡터 DB 없이 키워드 기반 검색. 오프라인/폐쇄망에서 동작합니다.
...
TUI 없이 커맨드라인에서 직접 실행. CI/CD, 셸 스크립트, 자동화에 활용.

```bash
# 단순 프롬프트
techai exec "이 프로젝트의 TODO를 정리해줘"

# 파이프 입력 (stdin → <stdin> 블록으로 LLM에 전달)
cat error.log | techai exec "이 에러를 분석해줘"

# git diff 자동 리뷰
git diff HEAD~1 | techai exec "코드 리뷰해줘"

# 커밋 메시지 자동 생성
git diff --staged | techai exec "커밋 메시지 써줘"

# 옵션
techai exec --ephemeral "일회성 질문"     # 세션 저장 안 함
techai exec --model "model-name" "질문"   # 모델 지정
techai exec --max-turns 5 "간단한 작업"   # 최대 도구 반복 제한
```

> 상세 문서: [docs/EXEC.md](docs/EXEC.md)
...
## Tools (15 built-in + MCP)

AI automatically invokes these tools during conversation. Shown as `Tool:ON(16)` in status bar.
...
| `apply_patch` | **멀티파일 패치**: 한 번의 호출로 여러 파일 Add/Update/Delete/Move. `@@` 함수 앵커로 위치 지정. 2+ 파일 수정 시 `file_edit`보다 3배 빠름 | Auto-snapshot + diff preview |
...
> `apply_patch` 상세 포맷: [docs/APPLY_PATCH.md](docs/APPLY_PATCH.md)

## Lifecycle Hooks

도구 실행 전후에 자동으로 셸 명령을 실행하는 확장 시스템. `.tgc/hooks.json` 또는 `~/.tgc/hooks.json`에 설정.
...
| 이벤트 | 시점 | 용도 |
|--------|------|------|
| `session_start` | 세션 시작 | 환경 검증, 로그 초기화 |
| `pre_tool_use` | 도구 실행 전 | 감사 로그, 위험 명령 차단 (exit 2 = abort) |
| `post_tool_use` | 도구 실행 후 | 자동 lint/format, 로깅 |
| `stop` | 턴 종료 | Slack/Teams 알림, 완료 노티 |
| `user_prompt` | 사용자 입력 | 입력 로깅 |

> 상세 문서: [docs/HOOKS.md](docs/HOOKS.md)
...
| **Exec Mode** | Headless CLI mode (`techai exec`). No TUI, outputs to stdout, supports piped stdin. For CI/CD and shell scripting |
| **Lifecycle Hooks** | User-defined shell commands that run before/after tool execution. Configured in `hooks.json`. Supports abort (exit 2) |
| **apply_patch** | Multi-file patch tool. One tool call to Add/Update/Delete/Move multiple files. Uses `@@` function anchors instead of line numbers |
...
| Global Hooks | `~/.tgc/hooks.json` | `C:\Users\<user>\.tgc\hooks.json` |
...
| Project Hooks | `.tgc/hooks.json` (overrides global) |
...
│   ├── tools/                   # AI 도구
│   │   ├── registry.go          # 도구 등록/실행 (15개 + MCP)
│   │   ├── file.go              # 파일 도구 (Fuzzy 4단계 편집)
│   │   ├── apply_patch.go       # 멀티파일 패치 (Codex 포맷)
...
│   ├── hooks/hooks.go           # Lifecycle Hooks (pre/post tool use)
│   ├── exec/exec.go             # 헤드리스 Exec 모드
│   ├── mcp/                     # MCP 클라이언트
