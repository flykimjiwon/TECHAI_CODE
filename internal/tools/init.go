...
// a comprehensive markdown profile suitable for .techai.md.
func GenerateProjectProfile(dir string) string {
	...
	tree := buildTree(absDir, "", 0)
	sb.WriteString(tree)
	sb.WriteString("```\n\n")

	// Dependencies
	deps := readDependencies(absDir, info.Type)
	if deps != "" {
	...
	// Entry points
	entries := findEntryPoints(absDir, info.Type)
	if len(entries) > 0 {
	...
	scripts := readScripts(absDir, info.Type)
	if scripts != "" {
	...
	// Knowledge packs (auto-detected from project type)
	packs := suggestKnowledgePacks(info)
	if len(packs) > 0 {
		sb.WriteString("## Knowledge Packs\n\n")
		sb.WriteString(fmt.Sprintf("knowledge_packs: %s\n\n", strings.Join(packs, ", ")))
		sb.WriteString("사용 가능한 팩: auth, bxm, charts, css, database, go, java, javascript, python, react, terminal, testing, tooling, typescript, utils, validation, vue\n\n")
	}
...
func buildTree(root, prefix string, depth int) string {
	...
	for i, e := range filtered {
		...
		if e.IsDir() {
			...
			sub := buildTree(filepath.Join(root, e.Name()), prefix+childPrefix, depth+1)
			sb.WriteString(sub)
		} else {
			sb.WriteString(fmt.Sprintf("%s%s%s\n", prefix, connector, e.Name()))
		}
...
func readDependencies(dir, projType string) string {
	switch projType {
	case "go":
...
func findEntryPoints(dir, projType string) []string {
	var entries []string
...
func readScripts(dir, projType string) string {
	var results []string
...
func readGitInfo(dir string) string {
	// Check if .git exists
...
// suggestKnowledgePacks returns recommended knowledge packs based on project type.
func suggestKnowledgePacks(info ProjectInfo) []string {
	packs := []string{"terminal"} // always useful

	switch info.Type {
	case "go":
		packs = append(packs, "go")
	case "node":
		packs = append(packs, "javascript", "typescript")
		switch info.Framework {
		case "nextjs":
			packs = append(packs, "react", "css")
		case "react", "vite":
			packs = append(packs, "react", "css")
		case "vue", "nuxt":
			packs = append(packs, "vue", "css")
		}
	case "python":
		packs = append(packs, "python")
	case "java":
		packs = append(packs, "java")
	}
